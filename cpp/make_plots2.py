...
for arg, match in zip(args, matches):
  ...
  for ii, tup in enumerate(match):
    ...
    if 'avg_samples' in opts:
      line, = plt.semilogy([it['avg_samples'] for it in iterations if 'avg_samples' in it], '%s.-' % colors[order % len(colors)], label='samples/100 (%s)' % name)
      handles_dict[order] = (line, name)
  ...
  plt.legend(handles_list, labels_list, loc=1,prop={'size':16})
  #plt.title(desc)
